 ...
 *
 * @param array  $order  rreshti salesorder (idso, ZINN, saltcode, title, quantity, unit, value, currency)
 * @param array  $cust   të dhënat e klientit nga tabela user (name, surname, email)
 * @param string $corrId correlation id
 * @return string XML kanonik
 */
function build_orders_canonical(array $order, array $cust, string $corrId): string
{
    ...
    $h->appendChild($doc->createElement('SenderId', 'ALBSALE_SALT'));    // NAD+SU
    ...
    $buyer->appendChild($doc->createElement('CustomerRef', (string)$order['ZINN']));  // ZINN -> CUSTOMER_ID
    ...
    $line->appendChild($doc->createElement('ProductRef', (string)$order['saltcode'])); // saltcode -> PRODUCT_ID
    $line->appendChild($doc->createElement('Description', (string)$order['title']));
    $line->appendChild($doc->createElement('Quantity', (string)$order['quantity']));   // QTY+21
    $line->appendChild($doc->createElement('Unit', (string)$order['unit']));
    $line->appendChild($doc->createElement('LineValue', (string)$order['value']));      // MOA+203
